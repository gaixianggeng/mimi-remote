...
| R12 | 原生 Harness 写路径与交互（#498 H07–H09）：提交结果未知被当成"没执行"而自动重发（一次提交变两次工具执行）；或审批/追问应答被伪造、被页面释放静默丢弃、在过期代次上复用 | 单元 + 契约；`HarnessDirectoryWiringTests`、`HarnessSessionJournalTests`、`HarnessSubmissionControllerTests`、`HarnessPresentationProjectorTests`、`HarnessEventClientTests`、`HarnessInteractionStoreTests` | 四组测试覆盖：链路失败必须判 `responseUnknown` 且不重发；`responseUnknown` 映射成 `.uncertain` 而非 `.rejected`；被取消的 attempt 不投影成助手消息；重复持久事件不产生第二条气泡；审批取值域外的结论、伪造的 question/option id 一律拒绝；页面关闭不是应答；撤权不记终态以便重投 | 用户的一次审批被应用两次，或一次授权请求被静默丢弃 |
