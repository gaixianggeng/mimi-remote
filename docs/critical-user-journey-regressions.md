...
| R12 | 原生 Harness 写路径与交互（#498 H07–H09）：提交结果未知被当成"没执行"而自动重发（一次提交变两次工具执行）；或审批/追问应答被伪造、被页面释放静默丢弃、在过期代次上复用；或应答写出成功即被当成"上游已接受"而提前撤卡 | 单元 + 契约；`HarnessDirectoryWiringTests`、`HarnessSessionJournalTests`、`HarnessSubmissionControllerTests`、`HarnessPresentationProjectorTests`、`HarnessEventClientTests`、`HarnessInteractionStoreTests`、`HarnessRecoveryCoordinatorTests`、`HarnessHostEventObserverTests`、`HarnessHistoryPageTests` | 五组测试覆盖：链路失败必须判 `responseUnknown` 且不重发；`responseUnknown` 映射成 `.uncertain` 而非 `.rejected`；被取消的 attempt 不投影成助手消息；重复持久事件不产生第二条气泡；审批取值域外的结论、伪造的 question/option id 一律拒绝；页面关闭不是应答；撤权不记终态以便重投；**撤卡只认带 eventId 的关联回执，写出成功不算接受**；**未打开会话的审批照样到达且按瀑布 agentId 归属**；**页面断开不退订宿主 `$events`、不关共享连接**；**历史分页要求 throughSeq 取自本次 follow，且游标不跨 runtime 混用** | 用户的一次审批被应用两次；一次授权请求被静默丢弃；或卡片提前消失让用户以为决定已生效 |
