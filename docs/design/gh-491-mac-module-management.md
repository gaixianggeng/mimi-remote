...
- 复用 `AgentDoctorResults.checks`（`name / ok / level / message / fix`）与现有 `runDoctor(fix:)`，
  面板只做投影，不新增探测逻辑。
...
| `diagnostics`「诊断与日志」 | **保留，但降为二级** | 摘要与修复入口进面板；窗口只承载完整报告、日志与导出 |
...
也就是说：**日常只用菜单栏**——看状态、开关模块、看额度、处理「需要登录 / 启用失败」、
看诊断结论并修可修项，都不用开窗口。常驻窗口只剩配对与设置，首次设置只出现一次。
