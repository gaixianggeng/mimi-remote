...
| 项目 | 当前状态 | 完成证据 / 下一步 |
| --- | --- | --- |
| Mac 一键启动 | 已完成菜单栏安装闭环 | 正式 Release 生成 Developer ID 签名并经过 Apple 公证的 universal DMG；菜单栏 App 内置 `agentd` 和 Claude bridge，负责首次设置、状态、配对、Doctor、登录启动和服务生命周期。检测到旧 Homebrew 服务时先保持其运行，用户确认后安全接管，失败自动尝试恢复旧服务；配置、移动端 Token 和配对关系继续保留。 |
| 服务存活与就绪分离 | 本轮完成 | `/healthz` 只表示进程存活；`/api/readyz` 除 Doctor 静态检查外，还会通过当前平台 transport 对 App Server 完成真实 `initialize`，失败返回 503。macOS SSH、Linux 共享 control socket 与 Windows 受管 WebSocket 探测都使用 single-flight。`up/start/restart` 通过前不展示二维码；`agentd status --json` 的 `process_ok` 对应存活，`service_ok` 对应 transport、版本和协议就绪。 |
| 安装、升级与回滚 | 本轮完成 | macOS 普通用户使用 DMG 覆盖升级，App 与配置分离，回滚优先恢复上一版已签名、公证 App；Homebrew 保留给命令行、服务器、自动化和恢复。Linux Release 内置 user-systemd 安装脚本，默认连接与终端共享的 Unix App Server，并在升级失败时恢复旧二进制和 unit。setup 原子写入配置；macOS/Linux 旧 managed WebSocket 配置只在对应共享 transport 预检通过后原子迁移。安全卸载只移除 Mimi 程序和服务，不停止共享 resident、Desktop 普通本地实例或 OpenClaw 实例。 |
...
| 敏感文件权限 | 本轮完成 | Doctor 检查配置必须是当前用户私有 regular file，并把权限收紧到 `0600`。Windows 的独立 App Server token 文件同样只允许当前用户读取。agentd 不保存 SSH 密码或私钥；macOS 的 host key、密钥和默认 Unix Socket，以及 Linux 默认 Unix Socket 都由当前用户管理。 |
...
| Gateway 协议边界 | 已完成 | Gateway 只放行移动端需要的公开方法，并按 schema 重建请求；`thread/search` 响应按 project、`browse_roots` 和 managed Worktree 的 cwd scope 逐条裁剪。标准审批和补充输入允许任一订阅入口首个响应；未知 Desktop 私有反向请求保持沉默。共享队列最低兼容 Codex `0.149.1`。 |
...
| 运行与诊断细节 | 本轮完成代码闭环 | `/api/diagnostics/relay` 提供限界现场数据；APP 已区分凭据终态、离线和瞬时失败，并保留会话/排队消息。`serve` 在 signal 或 HTTP 异常时先停止 HTTP 接入并 drain，再回收 agentd 自己的会话；macOS 关闭 SSH proxy、Linux 关闭本次网关连接，二者都保留共享 Unix App Server；Windows 停止自己托管的 loopback App Server。transport 或 App Server 异常会明确失败。剩余仅是真机 Wi-Fi/蜂窝、前后台和 Tailscale 暂停/恢复验收。 |
