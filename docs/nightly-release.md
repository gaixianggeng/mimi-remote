...
- 正式 iOS 候选通过 iOS CI 的 `workflow_dispatch` 和 `publish_app_store` 手工触发。正式 Mac 与 agentd 由维护者从已经进入 `main` 的 commit 创建并推送 `vX.Y.Z` tag，再发送 `repository_dispatch:release` 并传入该 tag；tag push 本身不读取发布 Secret。GitHub 固定从默认分支加载 Release workflow，workflow 再在不读取 Secret 的轻量 job 中核对官方仓库、受信 main SHA、目标 tag 与 `origin/main` 可达性，最后才允许签名和发布 job 进入 `production-release` Environment。MIM-207 暂停 Windows agentd 安装包发布。
