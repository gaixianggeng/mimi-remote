...
7. 首次启用或启用前已有 daemon 时，owner 都会持久标记为“待迁移”，并把磁盘 plist 固定为 `RunAtLoad=false`，不会静默中断既有进程，也不会在注销、登录或 Mac 重启后自动完成迁移。旧 `pid` backend、SSH/CLI 直启 listener 或签名父链不成立的 listener 都只能保持 attach 并收敛到 pending，不能因为它们来自官方 codex 就冒充 Browser 可用的 stable 状态；只有真实 socket peer 的签名 supervisor 拓扑通过才算 stable。普通 ensure 在每个 `bootout/bootstrap` 前都会重新探测 socket：即使 listener 在 codesign 或 plist 落盘期间才出现，也会立刻把 owner 降成 manual+marker，绝不对活跃 launchd coalition 执行 bootout。仅 durable fresh-enable intent 且 socket 尚未创建时允许 manual job 恢复一次冷启动，恢复后仍保持 pending；其他 pending 掉线只等待用户再次确认。
   ...
   用户确认后，Desktop 正在运行时先请求它正常退出。随后通过 App Server 协议分页读取全部已加载 task，并连续取得两份稳定快照；只有每个 task 都明确为 `idle` 才继续，`active`、`systemError`、`notLoaded`、未知状态或分页异常都 fail closed。managed `pid`、旧版未报告 PID 的 `pid` backend，以及 SSH/CLI 直启 listener 都必须绑定同一私有 socket、当前用户、官方可执行文件、argv、PID 启动时间、签名父链和 Desktop 最终退出状态。Desktop 复核后还会从真实 socket 再绑定一次 owner，避免向已经退出、被替换或复用的旧 PID 发信号。全部通过后，只向该 listener 发送一次 `SIGHUP`。Codex `>=0.141.0` 将它作为 graceful-only 信号：有活动 Turn 时继续等待，Turn 归零后才关闭传输并退出，重复 `SIGHUP` 也不会升级成强制终止。Mimi 不再调用会在 60 秒后强制终止的官方 `daemon stop`，也不会向共享 daemon 发送 `SIGTERM` 或 `SIGKILL`。等待旧 socket 持续拒绝连接 500 ms 且原 listener 的 PID/启动时间证据确认进程已经退出后，才 `bootout` 旧的已加载定义、`bootstrap` 磁盘上的 manual 新定义并 `kickstart` 一次。若 15 秒内仍有活动任务，操作返回“仍在优雅排空”，不会继续启动第二个服务；活动任务结束后可以重试。新进程必须完成协议、版本和运行态签名父链验证，全部通过后才清 intent/marker、把 plist 恢复为 `RunAtLoad=true`。上次已启动新签名 supervisor、但恰好在清 marker 前崩溃时，重试会直接提交健康进程，不再中断一次。任意失败都保留 manual+marker，不自动强杀或打开 Desktop；
...
移动端写入前，gateway 还会复用现有 external activity 的精确 Thread / Turn ownership 证据：同一 thread 确认由 Codex Desktop 运行时，`turn/start`、`turn/steer`、中断和其他写操作会在转发前返回 `accepted=false`；对 app-server 发给客户端的审批、补充输入和 MCP elicitation，移动端返回的反向 JSON-RPC response 也执行相同 guard，拒绝时保留 pending request 并恢复移动端卡片。`thread/resume` 与读取接口仍可用于只读观察。Desktop 仅打开空闲 thread 不会被当成占用。共享 Unix 模式下，每个会改变 task、thread、Turn、review 或 goal 的 gateway 请求都会持有共享使用锁，直到对应响应成功写回客户端或连接关闭；显式迁移持有同一文件的独占锁，因此不会在受控请求处理中途停止 daemon。每个连接最多保留 256 个未完成请求，非法或冲突的 JSON-RPC ID 直接拒绝。状态库不可读、观测器缺失、共享锁不可用或反向 request 缺少 thread scope 时会 fail-closed，暂时拒绝写入但继续允许读取；默认独立 WS 没有共同 writer，仍保持原有可用性策略。同一用户通过 SSH 直接连接 Unix socket 时不会经过 gateway 锁，所以最终安全边界仍是服务端 graceful-only `SIGHUP`，不是客户端空闲快照。
