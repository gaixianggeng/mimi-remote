...
新建 Codex 会话的自动标题使用一条独立的 App Server 连接（macOS 为 SSH proxy，Linux/Windows 为本机受管 WebSocket），不扩大移动端 allowlist。`agentd` 只在同一 Gateway 连接完成 `thread/start` 后消费首个成功转发的普通 `thread/queue/add`，用临时只读线程生成结构化标题，再通过 `thread/name/set` 写回目标线程。由于 app-server 只把该写操作的通知返回给内部连接，Gateway 会向发起会话的移动端补发同形的 `thread/name/updated`；完整边界见 [自动会话标题设计](auto-thread-titles.md)。
