...
历史读取固定使用 `thread/read(includeTurns:false)`，随后分页调用 `thread/turns/list` 和 `thread/items/list`。线程模型、工作目录和权限是共享状态；`thread/queue/add` 不隐式修改它们。移动端提交共享队列消息时，会先用独立的 `thread/settings/update` 一并提交 Composer 为下一回合明确选择的模型、推理强度、协作模式和权限，再调用 `thread/queue/add`。权限包含审批策略、审批人，以及互斥的沙盒或命名权限档案；提交权限时携带已授权 `cwd`，供 Gateway 收窄可写目录。沿用线程权限时省略权限覆盖字段，不用本地默认值改写已有配置。显式切换权限时，客户端同时等待设置请求成功和匹配的服务端权限快照。快照来自创建／恢复响应或 `thread/settings/updated`；上游不会为相同设置重复发通知，已有匹配快照时无需再等新通知。连接断开后丢弃快照；超时、取消或断线时不继续入队。工作区沙盒比较包含隐含可写的 `cwd`。界面将本地选择标为“下回合”，会话权限设置只展示服务端报告的值。共享队列协议没有每条消息的权限字段，因此不能保证其他客户端再次修改会话设置后，队列消息仍使用提交时的权限。
