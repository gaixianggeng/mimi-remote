...
- `0.2.12` 起 `thread/takeover` 只接管明确空闲（`status=idle`）的 Claude 会话。处理中（`busy` / `shell`）返回 `holder_busy`，缺失或未知状态返回 `holder_state_unknown`；两者可在状态更新后手动重试，不自动排队或中断当前轮。接管每次重新扫描全部外部持有方，任一不空闲就不发信号。全部空闲后仍须核实 session 登记、pid 存活、非进程池子进程，以及内核启动时间与登记 `startedAt` 相差 ≤10 秒。核实失败返回 `holder_unverified`。SIGINT → 等 5 秒 → 重新扫描并确认空闲及身份 → SIGTERM → 再等 5 秒，仍存活返回可重试的 `takeover_timeout`；不使用 SIGKILL。登记状态检查与发送信号不是 Claude 提供的原子交接协议，Mac 在最后一次检查之后瞬间开始新任务的极短窗口仍无法完全消除。
- 接管成功后同 id 续聊，按普通 resume 规则推迟到首个 `turn/start` 起进程，响应多一个 `takeover{released, holder, signal}`。持有方退出后有新 pid 认领时返回 `holder_respawned`，不继续发信号；无持有方、legacy 策略或进程池已持有时等价于普通 resume。agentd 保留 threadId + cwd 授权并强制 `excludeTurns`，只对 bridge >= `0.2.12` 声明 `thread/takeover` 和 `capabilities.idle_takeover=true`。iOS 同时要求方法与该能力；旧主机保持只读。输入框显示处理中提示，权威刷新到 idle 后恢复手动接管入口；失败原因按主机、会话、持有方保留。当前轮完成不要求先退出 Mac 上的 Claude，接管时才结束其空闲持有。反向分叉仍属于 #439 的独立范围。
