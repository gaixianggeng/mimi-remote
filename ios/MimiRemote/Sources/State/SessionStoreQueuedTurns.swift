...
/// turn 完成后回复正文缺失时的兜底补读。
///
/// turn/completed 是状态事件：它不经 ConversationStore 的序号水位线和修订门，也不依赖正文事件。
/// assistant 正文走的是另一条路——网关在客户端断线窗口只保留审批帧，其余一律丢弃；重连后又
/// 只回放状态级 backlog。于是「完成震动响了、气泡没出现」这种半状态可以稳定发生，而且不会自愈：
/// 只有手动刷新才会用权威 thread/read 把正文补回来。
///
/// 这里把手动刷新做的事自动化：当前可见会话的 turn 以 completed 收尾、本地却没有该 turn 的任何
/// assistant 正文时，先等一个合并窗口（让同批次的 messageCompleted 先落地），再安静地做一次
/// 强制权威读取。正文已经在的情况下什么都不做，所以正常链路没有额外请求。
extension SessionStore {
    ...
    func scheduleMissingAssistantReplyBackfillIfNeeded(
        ...
        missingAssistantReplyBackfillTasksBySessionID[sessionID]?.cancel()
        let lease = currentSelectionLease()
        let delay = Self.missingAssistantReplyBackfillDelayNanoseconds
        let task = Task { @MainActor [weak self] in
            do {
                try await Task.sleep(nanoseconds: delay)
            } catch {
            ...
            self.missingAssistantReplyBackfillTasksBySessionID[sessionID] = nil
            guard !self.isAppInBackground,
                  self.appStore.activeHostScope == hostScope,
                  self.isSelectionLeaseCurrent(lease),
                  !self.hasAssistantReply(sessionID: sessionID, turnID: turnID),
                  let session = self.sessionsByID[sessionID]
            else {
                return
            }
            _ = await self.loadHistory(
                for: session,
                quiet: true,
                force: true,
        ...
        missingAssistantReplyBackfillTasksBySessionID[sessionID] = task
    }

    func cancelMissingAssistantReplyBackfill(sessionID: SessionID) {
        missingAssistantReplyBackfillTasksBySessionID[sessionID]?.cancel()
        missingAssistantReplyBackfillTasksBySessionID[sessionID] = nil
    }
