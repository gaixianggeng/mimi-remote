...
extension ConversationDataFlowTests {
    ...
    func testWorkspaceSessionBranchPresentationHidesTheBaselineBranch() {
        // 13 行里 11 行是 main，只有一条 worktree 分支。main 不表达任何区别，
        // 只有偏离基线的那一行值得挂标签。
        var branches: [String?] = Array(repeating: "main", count: 11)
        branches.append("codex/customer-entity-import")
        branches.append(nil)

        XCTAssertEqual(
            WorkspaceSessionBranchPresentation.baselineBranch(among: branches),
            "main"
        )
        XCTAssertNil(
            WorkspaceSessionBranchPresentation.branchToDisplay("main", among: branches),
        ...
        XCTAssertEqual(
            WorkspaceSessionBranchPresentation.branchToDisplay(
                "codex/customer-entity-import",
                among: branches
            ),
            "codex/customer-entity-import",
            "偏离基线的那一行正是需要一眼看见的"
    ...
    func testWorkspaceSessionBranchPresentationShowsAllBranchesWithoutAUniqueMode() {
        // 众数并列时没有哪个分支称得上"默认"，全部照常显示。
        let branches: [String?] = ["main", "main", "codex/a", "codex/a"]

        XCTAssertNil(WorkspaceSessionBranchPresentation.baselineBranch(among: branches))
        XCTAssertEqual(
            WorkspaceSessionBranchPresentation.branchToDisplay("main", among: branches),
            "main"
        )
        XCTAssertEqual(
            WorkspaceSessionBranchPresentation.branchToDisplay("codex/a", among: branches),
            "codex/a"
        )
    }

    func testWorkspaceSessionBranchPresentationHidesTheOnlyBranchInAOneRowList() {
        // 只有一行时没有可对比的对象，挂一个标签纯属噪声。
        let branches: [String?] = ["codex/mim-97-fix-ipad-tab-return"]

        XCTAssertEqual(
            WorkspaceSessionBranchPresentation.baselineBranch(among: branches),
            "codex/mim-97-fix-ipad-tab-return"
        )
        XCTAssertNil(
            WorkspaceSessionBranchPresentation.branchToDisplay(
                "codex/mim-97-fix-ipad-tab-return",
    ...
    func testWorkspaceSessionBranchPresentationBaselineIgnoresBlankValues() {
        let branches: [String?] = [nil, "  ", " main\n", "main", "codex/a"]

        XCTAssertEqual(
            WorkspaceSessionBranchPresentation.baselineBranch(among: branches),
            "main",
            "空白项不参与词频，规范化后的同一分支应合并计数"
        )
        XCTAssertNil(
            WorkspaceSessionBranchPresentation.branchToDisplay(" main ", among: branches)
        )
