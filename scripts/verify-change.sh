...
if [[ "$direct_ios" == true ]]; then
  ...
  if [[ "$mode" == "full" ]]; then
    add_check "iOS full 只编译一次可复用测试产物" "bash ./scripts/ios-dev.sh build-for-testing"
    add_check "iOS full 复用产物执行核心关键链路回归" "IOS_TEST_ACTION=test-without-building bash ./scripts/test-conversation-regressions.sh"
    add_check "iOS full 复用产物执行英文文案 smoke" "IOS_TEST_ACTION=test-without-building bash ./scripts/test-ios-localization-smoke.sh"
  else
