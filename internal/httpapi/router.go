...
func NewRouterWithRuntimeInstallationIDAndOptions(
	...
	// external activity 必须读取与 Codex Desktop 相同的 CODEX_HOME。否则自定义
	...
	desktopBridge, bridgeErr := desktopipc.NewSystemBridge(
		cfg.Codex.DesktopSyncEnabled,
		cfg.AppServer.LegacyCleanupRequired,
		cfg.Codex.Bin,
		cfg.Codex.Env,
	)
	if bridgeErr != nil {
		log.Printf("Desktop IPC initialization failed")
		desktopBridge, _ = desktopipc.NewBridge(desktopipc.BridgeOptions{
			Enabled: cfg.Codex.DesktopSyncEnabled, InitialState: desktopipc.StateProtocolError,
		})
	}
