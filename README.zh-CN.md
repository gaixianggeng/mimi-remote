...
flowchart LR
    ...
    Codex["Codex<br/>受管 App Server + Desktop IPC overlay"]
...
- **真正的会话接力：**Codex 以独立受管 App Server 作为移动端主链路，并可为 Desktop 当前拥有的会话启用按版本限制的 Desktop IPC overlay；不会创建共享 daemon。
...
协议细节与准确能力边界见[项目现状](docs/project-status.md)、[Codex Desktop IPC](docs/codex-desktop-ipc.md)、[MIM-148 验收矩阵](docs/mim-148-acceptance-matrix.md)和 [Claude bridge 架构](docs/claude-bridge-architecture.md)。
...
`agentd up` 会生成用户私有配置和两层独立 Token，启动后台服务，等待真实 App Server WebSocket 就绪，然后在终端显示短期配对二维码。检测到 Tailscale 时优先使用 Tailscale；否则自动启用局域网，并把当前私有局域网地址写入配对信息。Desktop 会话同步默认关闭，启用后使用按版本限制的本机 IPC overlay；详见 [Codex Desktop IPC](docs/codex-desktop-ipc.md)。
