...
struct AgentCommandClient: Sendable {
    ...
    var configureCodexDesktopSync: @Sendable (
        _ enabled: Bool,
        _ cleanupLegacyConfirmed: Bool
    ) async throws -> CodexDesktopSyncConfigurationResult = { enabled, _ in
        CodexDesktopSyncConfigurationResult(enabled: enabled)
    }
...
extension AgentCommandClient {
    static func live(
        ...
        return AgentCommandClient(
            ...
            configureCodexDesktopSync: { enabled, cleanupLegacyConfirmed in
                let binary = try requireEmbeddedBinary()
                return try decode(
                    CodexDesktopSyncConfigurationResult.self,
                    from: try await execute(
                        binary: binary,
                        arguments: codexDesktopSyncConfigurationArguments(
                            enabled: enabled,
                            cleanupLegacyConfirmed: cleanupLegacyConfirmed
                        ),
                        timeout: .seconds(15)
                    )
                )
    ...
    static func codexDesktopSyncConfigurationArguments(
        enabled: Bool,
        cleanupLegacyConfirmed: Bool = false
    ) -> [String] {
        var arguments = [
            "runtime",
            "--codex-desktop-sync=\(enabled ? "enabled" : "disabled")",
            "--json",
        ]
        if cleanupLegacyConfirmed {
            arguments.append("--cleanup-legacy-sharing-confirmed")
        }
        return arguments
    }
