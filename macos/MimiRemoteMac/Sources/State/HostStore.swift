...
final class HostStore {
    ...
    private(set) var codexDesktopStatus = CodexDesktopSyncStatus()
    ...
    func setCodexDesktopEnabled(_ enabled: Bool) async {
        await configureCodexDesktopSync(enabled, cleanupLegacyConfirmed: false)
    }

    /// 旧共享模式只允许通过这一个明确入口清理。清理前由界面要求用户确认
    /// Desktop 已退出；普通同步开关不会退出或重新打开 Desktop。
    func cleanupLegacyCodexDesktopSync() async {
        await configureCodexDesktopSync(
            codexDesktopEnabled,
            cleanupLegacyConfirmed: true
        )
    }

    private func configureCodexDesktopSync(
        _ enabled: Bool,
        cleanupLegacyConfirmed: Bool
    ) async {
        guard !isUpdatingCodexDesktop else { return }
        guard owner == .macApp, !isBusy else {
            codexDesktopError = "请先启动并接管 Mimi Remote Mac 服务，再修改 Codex Desktop 同步设置。"
            return
        }
        isBusy = true
        ...
        do {
            let result = try await agent.configureCodexDesktopSync(
                enabled,
                cleanupLegacyConfirmed
            )
            if result.legacyCleanupRequired && !cleanupLegacyConfirmed {
                codexDesktopStatus = CodexDesktopSyncStatus(
                    enabled: result.enabled,
                    state: .legacyCleanupRequired
                )
                codexDesktopError = result.message
                return
            }
            if result.serviceRestartRequired {
                try await reloadMacAgentForConfigurationChange()
            }
            codexDesktopStatus = CodexDesktopSyncStatus(
                enabled: result.enabled,
                state: result.enabled ? .connecting : .disabled
            )
    ...
    private func apply(_ current: AgentStatus) {
        ...
        lastStatusRefreshAt = Date()
        codexDesktopStatus = codexRuntime?.desktopSync ?? CodexDesktopSyncStatus()
